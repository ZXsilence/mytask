...
def delete_keywords(keyword_delete_list, campaign_id, access_token, sid, nick):
    ...
    keywords_deleted = SimbaKeywordsDelete.delete_keywords(access_token, nick
...
def add_keywords(keyword_list, campaign_id, adgroup_id, access_token, sid, nick):
    if len(keyword_list) == 0:
        return []

    keywords_added = SimbaKeywordsvonAdd.add_keywords(access_token, nick, adgroup_id, keyword_list)
    keywords_added_dict = [k.toDict() for k in keywords_added]
    KeywordDBService.add_keyword_list(sid, nick, keywords_added_dict)
    return keywords_added_dict 

def update_keywords_price(keyword_list, campaign_id, access_token, sid, nick):
    if len(keyword_list) == 0:
        return []
    ...
    keywords_price_set_list = SimbaKeywordsPricevonSet.set_price(access_token, nick, keyword_price_toset_list)
    keywords_dict_list = [k.toDict() for k in keywords_price_set_list]
