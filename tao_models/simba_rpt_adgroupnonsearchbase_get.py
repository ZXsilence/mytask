...
class SimbaRptAdgroupnonsearchBaseGet(object):
    """
    """
    @classmethod
    @tao_api_exception(40)
    def get_rpt_adgroupnonsearchbase_list(cls, nick, campaign_id, adgroup_id, start_time, end_time, access_token, subway_token):
        """
        Notes:
                because of taobao API access-times limit,so we recommend that (end_time - start_time) do not more than a day
        """
        req = SimbaRptAdgroupnonsearchbaseGetRequest()
        req.nick = nick
        ...
        while True:  
            ...
            if not rsp.rpt_adgroup_nonsearch_base:
                l = {}
            else:
                l = json.loads(rsp.rpt_adgroup_nonsearch_base.lower())
            if l == {}:
                l = []
