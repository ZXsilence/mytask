...
from simba_campaign_area_get import SimbaCampaignAreaGet
from tao_models.common.exceptions import TaoApiMaxRetryException
from tao_models.common.exceptions import InvalidAccessTokenException
from TaobaoSdk.Exceptions import ErrorResponseException
@unittest.skipUnless('regression' in settings.RUNTYPE, "Regression Test Case")
class TestSimbaCampaignAreaGet(unittest.TestCase):
    '''
    ...
    def setUpClass(cls):
        ...
        cls.tcinfo = 'API Test - taobao.simba.campaign.area.get'
        cls.testInputDatas = [{'nick': '__NotExistNick__','campaign_id':3328400,'popException':True, 'exceptionClass':InvalidAccessTokenException},
        ...
        cls.itemFields = ['nick', 'create_time', 'campaign_id', 'modified_time', 'area'] #'nick,campaign_id,area'.split(',')
    ...
    def test_get_campaign_area(self):
        for inputdata in self.testInputDatas:
            self.tcinfo = 'API Test - taobao.simba.campaign.area.get'
            self.tcinfo += str(inputdata)
            is_poped = False
            try:
                returnValue = SimbaCampaignAreaGet.get_campaign_area(inputdata['nick'],inputdata['campaign_id'])
...
alltests = unittest.TestLoader().loadTestsFromTestCase(TestSimbaCampaignAreaGet)
