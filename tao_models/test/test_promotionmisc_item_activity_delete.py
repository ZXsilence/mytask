...
class TestPromotionmiscItemActivityDelete(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.testInputData = [{'nick':'chinchinstyle','activity_id':-1,'soft_code':'SYB','popException':False,'exceptionClass':None},#normal
                             {'nick':'','activity_id':-1,'soft_code':'SYB','popException':True,'exceptionClass':W2securityException},#nick is none 
                             ...
                             {'nick':'chinchinstyle','activity_id':0,'soft_code':'SYB','popException':True,'exceptionClass':W2securityException}, # activity_id is 0
        ...
        for inputdata in self.testInputData:
            ...
            try:
                if -1 == inputdata['activity_id']:
                    ...
                    self.assertEquals(type(res),int , 'reteurn type error !')
                    print 'The activity_id to delete is:', res
                    inputdata['activity_id'] = res
                res =  PromotionmiscItemActivityDelete.delete_item_activity(inputdata['nick'],inputdata['activity_id'],inputdata['soft_code'])
