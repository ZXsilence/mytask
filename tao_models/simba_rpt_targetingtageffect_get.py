...
from TaobaoSdk import SimbaRptTargetingtageffectGetRequest 
...
class SimbaRptTargetingtageffectGet(object):
    ...
    def get_rpt_adgroupnonsearcheffect_list(cls, nick, campaign_id, adgroup_id, start_time, end_time):
        ...
        req = SimbaRptTargetingtageffectGetRequest()
        req.nick = nick
        ...
        while True:  
            ...
            l = response['simba_rpt_targetingtageffect_get_response']['results']['rpt_effect_entity_d_t_o']
            if l == {}:
                l = []
            for rpt in l:
...
if __name__ == '__main__':
    ...
    rpt_list = SimbaRptTargetingtageffectGet.get_rpt_adgroupnonsearcheffect_list(nick, campaign_id, adgroup_id, start_time, end_time)
    for rpt in rpt_list:
        rpt['pay'] = float(rpt.get(u'directtransaction',0)) + float(rpt.get(u'indirecttransaction',0))
        print '%(crowdid)s,%(source)s,%(crowdname)s,%(pay)s'% rpt
