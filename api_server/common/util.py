...
def change_obj_to_dict_deeply(obj):
    ...
    for key, value in result_dict.iteritems():
        # 迭代转换
        if isinstance(value, object) and value.__class__.__module__.startswith('Domain'):
            result_dict[key] = change_obj_to_dict_deeply(value)
        elif isinstance(value, list):
            result_dict[key] = [change_obj_to_dict_deeply(obj) for obj in value]
    return result_dict
